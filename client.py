...
# Mode 1: Data Dummy Statis (Basic)
...
print("=======================================")
print("1. Mode Dummy Statis (Data berulang)")
print("2. Mode Dinamis Acak (Generate real-time)")
print("3. Mode Dataset Asli (Pakai file data_simulasi.csv)")
...
try:
    if pilihan == '3':
        # Baca dataset asli yang didownload dari Colab
        df = pd.read_csv('data_simulasi.csv')
        records = df.to_dict(orient='records')
        
        for row in records:
            # Karena IP dibuang saat training, kita suntikkan IP palsu untuk keperluan simulasi iptables
            if row['label_asli'] == 1:
                row['src'] = f"10.0.0.{random.randint(2, 255)}" # IP Luar
            else:
                row['src'] = f"192.168.1.{random.randint(10, 50)}" # IP Lokal
            ...
            time.sleep(0.1) # Kirim sangat cepat
